...
class SD2Trainer(BaseTrainer):
    ...
    def init_text_models(self):
        # 【改动1】保留初始化但不加载权重（最小改动，避免删除导致的依赖报错）
        self.tokenizer = None
        self.text_encoder = None
        logger.info("Text encoder/tokenizer disabled (empty prompt)")
    ...
    def encode_prompt(self, prompt: List[str]) -> Dict[str, torch.Tensor]:
        # 【改动2】无视输入prompt，直接生成空embedding（维度匹配SD2.x）
        batch_size = len(prompt) if prompt else 1  # 兼容传空/None的情况
